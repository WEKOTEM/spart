...
if we_sudo(msg) then
local text = '👋🏻 ~> اهلا بك مـطور العزيزي سررت بلقائك\n©  ~> انت المـطور الاساسي هنـا \n...\n\n✳️  ~> تسـتطـيع‌‏ تحكم في جميع اعدادات البوت \n📡  ~>  @VikaiQ  جـميع تحديثات فيـكاا هنـاا '
...
if data.forward_info_ then function infousers(arg,data) if data.username_ then user_name = '@'..data.username_ else user_name = data.first_name_ end sendMsg(arg.user_id,0,'['..r..']','md') sendMsg(msg.from.id,msg.id_,"📬¦ تم آرسـآل آلرسـآل‏‏هہ 🌿\n🎟¦ آلى : "..user_name.." 🏌🏻",'html') end tdcli_function({ID ='GetUser',user_id_ = data.forward_info_.sender_user_id_}, infousers, {user_id=data.forward_info_.sender_user_id_}) end end tdcli_function({ID ='GetMessage',chat_id_=msg.chat_id_,message_id_=data.id_},replay_fwd,nil) end tdcli_function({ID ='GetMessage',chat_id_=msg.chat_id_,message_id_=msg.reply_to_message_id_},get_msg_id,nil) end end if (msg.to.type == "pv") and not we_sudo(msg) and not redis:get(boss..'lock_twasel') and msg.from.id ~= our_id then -- ارسال رساله للاعضاء الي يدخلون خاص sendMsg(msg.to.id,0,"📬~>> تم ارسال رسالتك الى المطور\n📛~>> اشترك في قناة المطور \n🚸"..SUDO_USER,'md') forwardMessages(SUDO_ID,msg.to.id,{[0] = msg.id_},0) end if (r=="تيست" or r=="test") and is_sudo(msg) then return "عـزيزي البوت يعمل بكامـل قواى  @VikaiQ" end if (r== "ايدي" or r=="ايدي 🆔") and msg.to.type == "pv" then return "\n"..msg.from.id.."\n" end if r== "قناة السورس 📡" and msg.to.type == "pv" then local inline = {{{text="قنآهہ‏‏ آلسـورس : • •  فيكاأ☑️¦",url="t.me/tt3oiq"}}} return send_inline_key(msg.from.id,' [☑️¦قنآهہ‏‏ آلسـورس : • •  فيكاأ](t.me/t3oiq)',nil,inline)
end
...
return [[
🌿*~>* ها حبيبي هذه رابط ؛
😪 ~> مو بعد كم يوم تتندم ودكول كله من وراك ؛
👇🏼 ~> دوس عليها توديك لرابط حياتي ؛

🗑 ~> [أضغـط هناا ](https://telegram.org/deactivate) ؛

📡 ~> [قـناة السـورس](http://t.me/VIKAIQ) ؛
...
if is_sudo(msg) then  return   "حاضر تاج راسـي انجبيت 😇 "
